...
w: "Mo</td><td>Tu</td><td>We</td><td>Th</td><td>Fr</td><td>Sa</td><td>Su"
	...
	"ru": {
x: "Р—Р°РєСЂС‹С‚СЊ", nex: "РЎР»РµРґСѓСЋС‰РёР№ РјРµСЃСЏС†", pre: "РџСЂРµРґС‹РґСѓС‰РёР№ РјРµСЃСЏС†", clear: "РћС‡РёСЃС‚РёС‚СЊ",
m: ["РЇРЅРІ","Р¤РµРІ","РњСЂС‚","РђРїСЂ","РњР°Р№","РСЋРЅ","РСЋР»","РђРІРі","РЎРµРЅ","РћРєС‚","РќР±СЂ","Р”РµРє"],
mo: ["РЇРЅРІР°СЂСЊ","Р¤РµРІСЂР°Р»СЊ","РњР°СЂС‚","РђРїСЂРµР»СЊ","РњР°Р№","РСЋРЅСЊ","РСЋР»СЊ","РђРІРіСѓСЃС‚","РЎРµРЅС‚СЏР±СЂСЊ","РћРєС‚СЏР±СЂСЊ","РќРѕСЏР±СЂСЊ","Р”РµРєР°Р±СЂСЊ"],
w: "РџРЅ</td><td>Р’С‚</td><td>РЎСЂ</td><td>Р§С‚</td><td>РџС‚</td><td>РЎР±</td><td>Р’СЃ"
	}
}
var def = {
	lang: "en", // en/ru
...
HTM(a.id, '<thead><tr><td class="cal-l" title="'+lang[a.lang].pre+'">вЂ№</td><td colspan=3 class="cal-m"></td><td colspan=2 class="cal-y"></td><td class="cal-r" title="'+lang[a.lang].nex+'">вЂє</td></tr><tr><td>'+lang[a.lang].w+'</td></tr></thead>');

if(j>0) {// вЂ№ &larr;
	if(m==0) mm = 11; else mm = m-1;
	if(j>1) {
		ca += '<td colspan='+j+' class="cal-l" align=left title="'+lang[a.lang].pre+' '+lang[a.lang].mo[mm]+'"><b>вЂ№ '+lang[a.lang].m[mm]+'</b></td>';
	} else ca += '<td class="cal-l" title="'+lang[a.lang].pre+' '+lang[a.lang].mo[mm]+'"><b>вЂ№</b></td>';
}
...
if(DNlast>0) {// вЂє &rarr;
	if(m>10) mm = 0; else mm = m+1;
	if(DNlast<6) {
		ca += '<td colspan='+(7-DNlast)+' class="cal-r" align=right title="'+lang[a.lang].nex+' '+lang[a.lang].mo[mm]+'"><b>'+lang[a.lang].m[mm]+' вЂє</b></td>';
	} else ca += '<td class="cal-r" title="'+lang[a.lang].nex+' '+lang[a.lang].mo[mm]+'"><b>вЂє</b></td>';
}
